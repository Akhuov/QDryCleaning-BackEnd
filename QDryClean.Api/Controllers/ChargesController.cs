...
namespace QDryClean.Api.Controllers
{
    ...
    public class ChargesController : ControllerBase
    {
        ...
        [Authorize(Roles = $"{nameof(UserRole.Receptionist)},{nameof(UserRole.Admin)}")]
        [HttpDelete]
        public async Task<IActionResult> DeleteChargeAsync(CreateChargeCommand command)
        {
            var result = await _mediator.Send(command);
            return Ok(result);
        }
        [Authorize(Roles = $"{nameof(UserRole.Receptionist)},{nameof(UserRole.Admin)}")]
        [HttpPut]
        public async Task<IActionResult> UpdateChargeAsync(CreateChargeCommand command)
        {
            var result = await _mediator.Send(command);
            return Ok(result);
        }
